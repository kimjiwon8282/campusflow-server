...
@Schema(description = "희망과목 담기 대상 강의 응답. 실제 수강신청 확정 정보가 아닙니다.")
public record StudentWishCourseCandidateResponse(
    ...
    Long autoApplyCount,
    @Schema(description = "현재 학생이 이미 희망과목으로 담았는지 여부", example = "true")
    boolean wished,
    @Schema(description = "현재 학생의 자동신청 선택 여부", example = "true")
    boolean autoApply,
    @Schema(description = "현재 학생의 자동신청 결과. 담지 않은 강의는 null", example = "PENDING")
